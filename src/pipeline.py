...
def retrieve_documents(
    ...
    try:
        ...
        logger.debug(f"Embedding query: '{prefixed_query}'")
        ...
        logger.debug(f"Performing similarity search by vector with k={k}")
        ...
        retrieved_ids = []
        logger.debug("--- Extracting IDs from retrieved documents ---")
        for i, doc in enumerate(retrieved_docs):
            doc_id = doc.metadata.get('original_id', None)
            if doc_id is not None:
                retrieved_ids.append(str(doc_id))  # Ensure ID is string
            else:
                logger.warning(f"Retrieved Doc {i+1} has missing 'original_id' in metadata: {doc.metadata}")
            logger.debug(f"  Doc {i+1} Metadata: {doc.metadata}")
            logger.debug(f"  Extracted ID: {doc_id} (Type: {type(doc_id)})")
        logger.debug("--- Finished extracting IDs ---")
        
        # Log retrieval results
        logger.debug(
            f"Retrieved {len(retrieved_docs)} documents in {latency:.4f} seconds. IDs: {retrieved_ids}"
        )
        logger.info(
            f"Document retrieval completed: {len(retrieved_docs)} docs, latency={latency:.4f}s"
        )
        
        return retrieved_docs, retrieved_ids, latency
        
    except Exception as e:
        logger.error(f"Document retrieval failed: {e}", exc_info=True)  # Added exc_info
        return [], [], 0.0
...
def run_rag_chain(
    ...
    try:
        ...
        logger.info(f"Starting RAG chain for question: '{question_text}'")
        ...
        try:
            ...
            logger.info("Invoking generator LLM with formatted prompt")
            ...
            logger.info(f"Answer generation completed in {generation_latency:.4f} seconds")
            logger.debug(f"Generated answer: {generated_answer[:100]}..." if 
                         len(generated_answer) > 100 else generated_answer)
